...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (match) {
    const uniDropdown = Array.from(
      document.querySelectorAll(".main-nav > ul > li.dropdown")
    ).find(
      (li) =>
        li.querySelector("a") &&
        li.querySelector("a").textContent.trim().toLowerCase() === "university"
    );
    if (uniDropdown) {
      const label = pageToLabel[match];
      const targetLink = Array.from(
        uniDropdown.querySelectorAll(".dropdown-content a")
  ...
  footer.innerHTML = `
    <div class="footer-content">
      ...
      <div class="footer-column footer-info">
        ...
        <p>Devarakaggalahalli, Harohalli, Kanakapura Road,<br />Bengaluru South
            Dt. – 562 112</p>
        <p>E-mail: <a href="mailto:admissions@dsu.edu.in">admissions@dsu.edu.in</a></p>
...
document.addEventListener("DOMContentLoaded", () => {
  const path = window.location.pathname.toLowerCase();
  const researchTop = Array.from(
    document.querySelectorAll(".main-nav > ul > li.dropdown > a")
...
document.addEventListener("DOMContentLoaded", () => {
  const path = window.location.pathname.toLowerCase();
  const academicsDropdown = Array.from(
    document.querySelectorAll(".main-nav > ul > li.dropdown")
  ).find(
    (li) =>
      li.querySelector("a") &&
      li.querySelector("a").textContent.trim().toLowerCase() === "academics"
  );
  ...
  if (current) {
    const target = Array.from(list.querySelectorAll("a")).find(
...
document.addEventListener("DOMContentLoaded", () => {
  const path = window.location.pathname.toLowerCase();
  const admissionsDropdown = Array.from(
    document.querySelectorAll(".main-nav > ul > li.dropdown")
  ).find(
    (li) =>
      li.querySelector("a") &&
      li.querySelector("a").textContent.trim().toLowerCase() === "admissions"
  );
...
document.addEventListener("DOMContentLoaded", () => {
  const path = window.location.pathname.toLowerCase();
  const researchDropdown = Array.from(
    document.querySelectorAll(".main-nav > ul > li.dropdown")
  ).find(
    (li) =>
      li.querySelector("a") &&
      li.querySelector("a").textContent.trim().toLowerCase() === "research"
  );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const dsuResponses = {
    admission:
      "DSU admissions open for 2024-25. Apply online through KCET/COMEDK/Management quota.",
    courses:
      "Engineering: CSE, ECE, Mech, Civil, EEE. Other: BBA, BCA, B.Com, MBA, MCA, M.Tech.",
    ...
    mechanical:
      "Mechanical Engineering: 4 years, Fee: ₹2.2L/year, Modern labs.",
    ...
    scholarship:
      "Merit scholarships, SC/ST fee concession, Sports quota available.",
    hostel:
      "Separate hostels for boys and girls. AC/Non-AC rooms. Fee: ₹80K-1.2L/year.",
    placement:
      "90%+ placement rate. Top recruiters: Infosys, TCS, Wipro, Accenture, Amazon.",
    contact: "Phone: +91-80-28447844, Email: info@dsu.edu.in",
    address:
      "Dayananda Sagar University, Harohalli, Main Campus, Bangalore - 560078",
    location:
      "Located in Bangalore, Karnataka. Well connected by metro and bus.",
    campus:
      "60-acre green campus with modern infrastructure, labs, and sports facilities.",
    library:
      "Central library with 1L+ books, e-resources, and digital learning center.",
    labs: "State-of-the-art labs for all departments with latest equipment.",
    sports:
      "Cricket, Football, Basketball, Volleyball, Badminton, Gym facilities.",
    ...
    research:
      "Active research in AI, IoT, Robotics, and emerging technologies.",
    cst: "Computer Science And Technology , 4 years, Fee: ₹2.5L/year, NAAC A+ accredited. ",
    default:
      "I can help with DSU admissions, courses, contact details, fees, and placements. What would you like to know?",
  };
  ...
  function getBotResponse(message) {
    ...
    if (lowerMessage.includes("admission") || lowerMessage.includes("apply"))
      return dsuResponses.admission;
    if (
      lowerMessage.includes("cse") ||
      lowerMessage.includes("computer science")
    )
      return dsuResponses.cse;
    if (lowerMessage.includes("ece") || lowerMessage.includes("electronics"))
      return dsuResponses.ece;
    if (lowerMessage.includes("mechanical") || lowerMessage.includes("mech"))
      return dsuResponses.mechanical;
    ...
    if (lowerMessage.includes("event") || lowerMessage.includes("fest"))
      return dsuResponses.events;
    if (lowerMessage.includes("accreditation") || lowerMessage.includes("naac"))
      return dsuResponses.accreditation;
    if (lowerMessage.includes("faculty") || lowerMessage.includes("teacher"))
      return dsuResponses.faculty;
  ...
  document.querySelectorAll(".quick-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      setTimeout(
        () => addMessage(dsuResponses[query] || dsuResponses.default, "bot"),
        500
      );
    });
  });
